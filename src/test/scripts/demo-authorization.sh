...
checkAccessFromOtherNs() {
    ...
    if kubectl exec -n foo "$pod" -c istio-playground -- curl -s http://$2-app-service.quarkus:8000/login.html | grep -q "$($3_pattern)"; then
        printf "$CHECKMARK\n"
    else
        printf "$CROSS\n"
        exit 1
...
checkAccess() {
    ...
    if kubectl exec -n quarkus "$pod" -c istio-playground -- curl -s http://$2-app-service.quarkus:8000/login.html | grep -q "$($3_pattern)"; then
        printf "$CHECKMARK\n"
    else
        printf "$CROSS\n"
        exit 1
